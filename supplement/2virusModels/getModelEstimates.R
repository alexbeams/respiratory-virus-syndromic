...
## assign julian dates (uncomment this after loading the data:
#dat$julian <- as.numeric(as.Date(dat$StartTime))
...
source('ModelFiles/getModEst.R')

## assign useful names for the pathogens (uncomment if the data is available):
#pnms <- names(dat)[7:28]
#goodpnms <- pnms[1:20] #B..parapertussis and SARS.CoV.2 have lots of NAs

#generate all the unique pairs - 190 of them
#pairnms = rep(0,190)
#pairlist = list()
#k=1
#for(i in 1:19){
...
#	for(j in (i+1):20){
#		path2nm=goodpnms[j]
#		pathpair=paste0(path1nm,'_+_',path2nm)
#		pairnms[k]=pathpair
#		pairlist[[k]]=list(path1nm,path2nm)
#		k=k+1
#	}
#}
#
...
getCoinfDat <- function(pairs){
	coinfdat <- lapply(pairlist, function(x) getddat(x[[1]],x[[2]]))
	return(coinfdat)

}


# running sapply(coinfdat, function(x) getlogl(x,theta_init)) -> crud confirms the starting value is ok for each (no NAs, Infs, etc)
library(parallel)

plotCoinfDat <- function(dat){
	plot(dat[,2],ylim=c(0,200))
	points(dat[,1],col='purple')
	points(dat[,3],col='blue')
	points(dat[,4],col='red')

}

plotSimDatPred <- function(theta,new=F){
	plot(phi1,loglik,xlab=expression(phi), ylab='log-likelihood',ylim=c(lmax-20,lmax+5))
	simdat <- getSimDat(theta)
	if(new==T){plot(simdat$x00)}else{lines(simdat$x00)}
	lines(simdat$x11,col='purple')
	lines(simdat$x10,col='blue')
	lines(simdat$x01,col='red')
}

plotFit <- function(dat,theta){
	plotCoinfDat(dat)	
	simdat <- getSimDat(theta)
	lines(simdat$x00)
	lines(simdat$x11,col='purple')
...
plotFit2 <- function(dat,theta){
	simdat=getSimDat(theta)	
	par(mfrow=c(2,2))
	plot(dat[,2],ylim=c(0,200),col='gray',main='Negative')
	lines(simdat$x00,col='black')	
	plot(dat[,1],col='black',main='Coinfection')	
	lines(simdat$x11,col='purple')
	plot(dat[,3],col='black',main='Path 1 only')	
	lines(simdat$x10,col='blue')
	plot(dat[,4],col='black',main='Path 2 only')
	lines(simdat$x01,col='red')
}

plotFitSim1 <- function(dat,theta,DT=1/7,wndw=52*6,tEnd=8000,Y=runif(9),save=F,filnm=F,pathnm='Pathogen',start=16801,end=18199){
...
plotFitMean1 <- function(dat,theta,DT=1/7,wndw=52*6,tEnd=8000,Y=runif(9),save=F,filnm=F,pathnm='Pathogen',start=16801,end=18199,Npop=327e6,tExtra=0,phase=0,add=F){
	...
	sstot = list(pos = sum((dat[,1]-mean(dat[,1]))^2), neg = sum((dat[,2]-mean(dat[,2]))^2))
	return(list(rss_mod,rss_supsmu,sstot))
}

plotFitMeanPos <- function(dat,theta,DT=1/7,wndw=52*6,tEnd=8000,Y=runif(9),save=F,filnm=F,pathnm='Pathogen',start=16801,end=18199,Npop=327e6,tExtra=0,phase=0,pathcol='black',lin1=18199){
	# this is code is idiotically written. tEnd is measured in weeks, but start and end are julian days.
	# the point is that we simulated the ode for 8000 weeks to make sure we're at an attractor,
	# and the fitting procedure involved choosing parameters so that the last 1399 days (199.8571 weeks) 
	# of that 8000-week long simulation match the data. So, with the default tEnd=8000, day 1 of the data 
	...
	if(save==T){pdf(file=filnm,height=8,width=8)}
	#par(mfrow=c(2,1))
	plot(as.Date(tmslbl),dat[,1],ylim=c(0,1.3*max(dat[,1])),col=pathcol,main=paste0(pathnm,' Positive'),ylab='No. of tests',
		xlab='Time [years]',type='p',cex=1.4)
...
plotFitMeanReg1 <- function(pathnm,theta,DT=1/7,wndw=52*6,tEnd=8000,Y=runif(9),save=F,filnm=F,start=16801,end=18199,Npop=327e6){
...
plotFitResiduals1 <- function(dat,theta,DT=1/7,wndw=52*6,tEnd=8000,Y=runif(9),save=F,filnm=F,pathnm='Pathogen'){
...
trylogl1 <- function(x,theta,Y=runif(9), tEnd=8000){
	logl1 <- try({
...
# we can probably use the same initial conditions every time...
initList <- list()
for(i in 1:20) initList[[i]] <- runif(9)
rm(i)
...
plotSimPrevEns <- function(ensemble,pathnm,start=16801,end=18199,tEnd=8000,filnm=F,save=F,nsims=10,Y=runif(9),a=0,b=1){
	...
	lines(log10(r1)~as.Date(tms),sim,lty='dotted',col='darkgreen')
	lines(log10(i1)~as.Date(tms),sim,col='red')
	...
	for(i in 2:nsims){
		...
		sim=tail(sim,n=length(tms))	
		lines(log10(s1)~as.Date(tms),sim,lty='dashed',col='blue')				
		lines(log10(r1)~as.Date(tms),sim,lty='dotted',col='darkgreen')
		lines(log10(i1)~as.Date(tms),sim)	
	}
	if(save==T){dev.off()}
...
iavnms = c('Influenza.A.all','Influenza.A.H1.2009','Influenza.A.H3','Influenza.A..no.subtype')
...
plotDisruption <- function(ensemble,pathnm,start=16801,end=18199,tEnd=8000,filnm=F,save=F,Y=runif(9),a=0,b=0,thresh=1,xmax=1500,simmax=1000){
	# just select the end of the mcmc:	
	theta=getmcmcmle(ensemble)
	lmax=theta[length(theta)]	
	# find the parameters that fall between 11 and 9 log-likelihoods of the MLE:	
