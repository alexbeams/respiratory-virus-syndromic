...
simdat = cbind(simprev$t, simdat)
simmean = cbind(simprev$t, simmean)
colnames(simdat) <- c('t','y','x')
colnames(simmean) <- c('t','y','x')


par(mfrow=c(1,2))
# Plot the simulated data:
plot(x~t,simdat, xlab='Time [wks]', ylab='No. of detections',
	main='Simulation Data')
points(y~t,simdat, col='red')
lines(x~t,simmean,col='black', lwd=3)
lines(y~t,simmean,col='black', lwd=3)
