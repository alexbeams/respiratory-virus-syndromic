...
# read in the MCMC output and store MLEs, 95% CIs, and standard errors:
...
# store the names of the files with MCMC output:
...
#goodsims <- sapply(simnms, function(x){load(paste0('simdatfits/',x));dim(ens)[2]==2000})
#goodsims <- simnms[goodsims]

# store statistics from the MCMC output in a list
...
# make some vectors of estimates vs true values
# for plotting
...
## we can use plotEpiPar and plotPsiPar to see if the ensemble MCMC has converged:

source('plotfunctions.R')

# example: this will load an array called ``ens'':
load('Figure2/simdatfits/simdat1.Rdata')

plotEpiPar(ens)

plotPsiPar(ens)

# ^^ run for more timesteps if the profile loglikelihoods aren't resolved
